...
void erpc_packet_zero(erpc_packet_t *packet) {
    packet->length = 0;
...
bool erpc_packet_is_valid(erpc_packet_t *packet) {
    ...
    if (packet->data[0] != ERPC_MAGIC_BEGIN) {
        erpc_packet_zero(packet);
        return false;
    }
    if (packet->length < 5) {
        return false;
    }
    if (packet->length > 260) {
        erpc_packet_zero(packet);
        return false;
    }
    uint8_t data_length = packet->data[1];
    if(packet->length != data_length + 5) {
        return false;
    }
    if (packet->data[data_length + 2 + 2] != ERPC_MAGIC_END) {
        erpc_packet_zero(packet);
        return false;
    }
    uint16_t crc = *(uint16_t *) (packet->data + 2 + data_length);
    uint16_t crc_local = erpc_crc16(packet->data + 2, data_length);
    if (crc != crc_local) {
